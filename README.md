...
v0.6.1 修复连续学习的真实声音链路：英文和 Windows 中文释义统一先形成 PCM WAV，再通过 Windows `waveOut` 播放；单独试听、停止事件和自动下一词共用同一可靠播放器。v0.6.0 的智能文档解析、专注学习、Anki、备份和旧数据保持兼容。
...
- [[单词文档配音项目交接文档-2026-07-22]]：v0.6.1 连续播放修复、智能解析、兼容边界与发布结果。
...
v0.6.1 构建结果位于 `dist\WordPdfVoice-v0.6.1\WordPdfVoice-v0.6.1.exe`。整个版本文件夹需要一起保留；构建前会检查源码、配置和正式文件名的版本号是否一致。

构建结束会自动调用成品程序生成 Sarah、Adam、慢速和快速真实 WAV；声音或语速没有产生实际差异时，构建会直接失败。

当前 v0.6.1 继续内置离线英文发音模型，并调用 Windows 已安装的离线中文语音；首次使用和日常发音都不依赖网络，OCR 扩展不在基础包内。

v0.2 以来的项目数据继续保存在 `%LOCALAPPDATA%\WordPdfVoice\v0.2\projects\`。v0.6.0 首次打开旧库时先自动备份，再增加解析来源字段；旧词条、人工编辑、熟悉度和音频无需重新生成，具体约定见 [`docs/versioning-and-data.md`](docs/versioning-and-data.md)。
