...
v0.6.0 增加智能文档解析：规则四六级词表继续使用快速表格提取，双栏中英笔记使用坐标分栏并在确认窗口中校对后导入。分析支持逐页进度与取消，复杂结果保存页码、坐标和置信度；原有专注学习、连续播放、Anki、音频和备份能力保持兼容。
...
- [[单词文档配音项目交接文档-2026-07-22]]：v0.6.0 智能解析、兼容边界、验证与发布结果。
...
v0.6.0 构建结果位于 `dist\WordPdfVoice-v0.6.0\WordPdfVoice-v0.6.0.exe`。整个版本文件夹需要一起保留；构建前会检查源码、配置和正式文件名的版本号是否一致。

构建结束会自动调用成品程序生成 Sarah、Adam、慢速和快速真实 WAV；声音或语速没有产生实际差异时，构建会直接失败。

当前 v0.6.0 便携版约 373.2 MiB，包含离线发音模型，因此首次使用和日常发音都不依赖网络；OCR 扩展不在基础包内。
